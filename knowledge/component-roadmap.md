...
components are marked done in Diseño; **Asistente** remains pending in Desarrollo.
Empty, Skeleton, Calendar card, and Side bar are now implemented in code (see gap
table).

Cross-checked against the actual code (`packages/ui/src/components/*`,
`packages/ui/src/patterns/*`) on 2026-07-22: **1** pending timeline item remains
without a code export. Empty (`Empty.tsx`), Skeleton (`Skeleton.tsx`),
CalendarCard (`CalendarCard.tsx`), and SideBar (`SideBar.tsx`) ship. The 6 items
marked done in both rows (Tarjetas → `Card`/`TaskCard`, Investigation card →
...
1. **Asistente** — highest complexity (chat UI, dynamic per-user suggestions,
...
(p.15); `SideBar` (p.13) — controlled collapsible nav rail with declarative items.
...
etc.) was masking this same gap. **Side bar** (`#282828`) draws its own opaque background
so this shouldn't recur, but **Skeleton** (`#2a2927` at 70% opacity — not fully opaque)
and **Asistente** should get an explicit dark decorator background in their stories from
the start rather than depending on `parameters.backgrounds`.
