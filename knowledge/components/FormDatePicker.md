...
| 0.1.1 | Fidelity pass de geometría 2026-08-10: el offset del panel (`calc(100% + 2px)`/`-1px`) era inventado — p.21 mide un gap real de ~0 (panel y trigger contiguos), ahora `top:100%`/`left:0`. Hereda además los fixes de `.ds-form-field__control`/`__label` de `FormTextInput` (altura del trigger, inset del label). |
